...
type R (p:P) =
    ...
    member val Comments         = CommentRegex(p.Comments)
    member val Directives       = DirectiveRegex(p.Directives)
    member val Instructions     = InstructionRegex(p.Instructions)
    member val Opcodes          = OpcodeRegex(p.Opcodes)
    member val Operands         = OperandRegex(p.Operands)
    member val Immediates       = ImmediateRegex(p.Immediates)
    member val Labels           = LabelRegex(p.Labels)
    member val BasePlusOffsets  = BasePlusOffsetRegex(p.BasePlusOffsets)
    member val Registers        = RegisterRegex(p.Registers)
...
and CommentRegex(cp:CommentPatterns) =
    member val CommentOnly = Regex(cp.CommentOnly)

and DirectiveRegex(dp:DirectivePatterns) =
...
and LabelRegex(lp:LabelPatterns) =
    member val New      = Regex(lp.New)
    member val Inline   = Regex(lp.Inline)

and BasePlusOffsetRegex(bpop:BasePlusOffsetPatterns) =
...
and RegisterRegex(rp:RegisterPatterns) =
...
let (|Directive|_|) (encoder:DLXEncoder) : DirectiveRegex * string -> string option = 
...
let (|Instruction|_|) (encoder:DLXEncoder) : InstructionRegex * string -> string option =
    let instruction r s = let g = groups r s in (g.[1].Value, g.[1].Value)
    let encode r s = s |> instruction r ||> encoder.Encode |> Some
    function
    | _ -> None

let (|Opcode|_|) (encoder:DLXEncoder) : OpcodeRegex * string -> string option =
    let opcode r s = let g = groups r s in (g.[1].Value, g.[1].Value)
    let encode r s = s |> opcode r ||> encoder.Encode |> Some
    function
    | _ -> None

let (|Operapnds|_|) (encoder:DLXEncoder) : OperandRegex * string -> string option =
    let opcode r s = let g = groups r s in (g.[1].Value, g.[1].Value)
    let encode r s = s |> opcode r ||> encoder.Encode |> Some
    function
    | _ -> None

let (|Immediate|_|) (encoder:DLXEncoder) : ImmediateRegex * string -> string option =
...
let (|Label|_|) (encoder:DLXEncoder) : LabelRegex * string -> string option =
...
let (|BasePlusOffset|_|) (encoder:DLXEncoder) : BasePlusOffsetRegex * string -> string option =
    let opcode r s = let g = groups r s in (g.[1].Value, g.[1].Value)
    let encode r s = s |> opcode r ||> encoder.Encode |> Some
    function
    | _ -> None

let (|Register|_|) (encoder:DLXEncoder) : RegisterRegex * string -> string option =
