...
//        info |> List.fold (fun r s -> 
//            let s = s |||> sprintf "%s, %s, %s"
//            r + s + "") ("")

type DLXEncoder =
    { EncoderMap : EncoderMap }
    member e.Encode (key:string) (target:string) = 
        target |> e.EncoderMap.[key]

    static member create (el:(string*EncoderFunc) list) =
        { EncoderMap = el |> Map.ofList }
and EncoderMap = Map<string, EncoderFunc>
and EncoderFunc = string -> string
and EncoderInput = Key * Target
and Key = string
and Target = string
...
//    | JType of string
//    
//    member o.Encode(?opcodes:Opcodes) = 
//        let opcodes = defaultArg opcodes (Opcodes())
...
//
////    member o.ToBinary(?opcodes:Opcodes) = 
////        let opcodes = defaultArg opcodes (Opcodes())
////        o.Encode(opcodes)
////
////    member o.ToHex() = 
